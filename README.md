...
- The default Q-table is a demo table, not a trained policy.
- The controller uses a greedy policy only.
- The flight target is currently fixed to the far grid corner.
- Safety checks are minimal and should be improved before more aggressive testing.
- The scripts assume Qualisys is the source of truth for position.

## Troubleshooting

If the hover test or Q-learning flight does not start correctly, check these first:

- QTM is running and reachable at `QTM_IP`
- the rigid body name in QTM matches `MARKER_DECK_NAME`
- the active marker IDs match the hardware deck
- the Crazyradio URI is correct
- the pose stream is arriving before estimator reset
- the grid bounds and hover altitude make sense for the physical space

If the script times out waiting for mocap data, focus on the Qualisys connection and rigid body naming first.

## Safety Notes

- Clear the flight volume before arming.
- Start with the hover test after any hardware or marker changes.
- Keep grid bounds conservative until the frame alignment is verified.
- Treat the generated fake Q-table as a demo only, not as a safe production controller.
